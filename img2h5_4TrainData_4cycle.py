...
#python img2h5_4TrainData.py -i ./data -o boy_trainG -t gray
...
def main():
    ...
    parser.add_argument('--inpath1', '-i1', required=True)
    parser.add_argument('--inpath2', '-i2', required=True)
    parser.add_argument('--outpath', '-o', required=True)
    parser.add_argument('--trans', '-t', default='raw')
    args = parser.parse_args()
    img_rows =128
    img_cols =128

    finders = glob.glob(args.inpath1+'/*')
    print(finders)
    imgs = []
    for finder in finders:
        ...
        for imgfile in files:
            ...
            if args.trans=='gray':
                ...
                print("gray_canny_xy",gray_canny_xy.shape)
                imgs.append(gray_canny_xy.reshape(128,128,1))                
                    
    finders = glob.glob(args.inpath2+'/*')
    print(finders)
    gimgs = []
    for finder in finders:
        files = glob.glob(finder+'/*')
        for imgfile in files:
            if args.trans == 'raw':
                gimg = load_img(imgfile, target_size=(img_rows,img_cols))
                imgarray = img_to_array(gimg)
                gimgs.append(imgarray)
            if args.trans=='gray':
                grayimg = load_img(imgfile, grayscale=True, target_size=(img_rows,img_cols))
                grayimgarray = img_to_array(grayimg)
                gimgs.append(grayimgarray)
            elif args.trans=='canny':
                gimg = image.load_img(imgfile, target_size=(img_rows,img_cols))
                gimg.save(imgfile)
                #grayimg = cv2.cvtColor(cv2.imread(imgfile, cv2.COLOR_BGR2GRAY)) #imgfileのサイズに依存しないように変更
                grayimg = cv2.imread(imgfile,0)                
                print("gray_canny_xy",grayimg.shape)
